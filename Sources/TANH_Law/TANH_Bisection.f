...
C JC yield stress
C JC part 1
      JCPart1 = parA + parB * epsp**parn
C JC part 2
      if (depsp .gt. pardepsp0) then
        JCPart2 = 1.0 + parC * log (depsp/pardepsp0)
      else
        JCPart2 = 1.0
      end if
C JC temp part
...
C Compute and return the JCStress
      JCStress = JCPart1 * JCPart2 * tempPart
C Hardening part of the Johnson-Cook law
      hardPart = parn * parB * (epsp**(parn - 1.0))
C Dependence to the deformation rate
      if (depsp .gt. pardepsp0) then
        hardPart = hardPart * (1.0 + parC * log (depsp/pardepsp0))
     1    + parC * (parA + parB * epsp**parn)/(depsp*dt)
      end if  
C tanh(T)
      tanhT = tanh(((temp - parT0) / (parTrec - parT0))**parq)
C partD
...
C Compute and return the yield stress
      yieldHard = hardPart * tempPart * ( partD + (1.0 - partD) *
     1   tanh(1.0 / ( epsp + epsp00 ))) + JCStress * 
     2   ((tanh(1.0/(epsp + epsp00))-1.0) * parp/(parp*epsp+1.0)**2.0
     3   * tanhT +(partD-1.0)* sech2e /(epsp+epsp00)**2.0)
      return
      end
