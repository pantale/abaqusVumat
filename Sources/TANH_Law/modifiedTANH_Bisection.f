C **********************************************************************
C Function to compute the  modified TANH yield stress
C **********************************************************************
      function yieldStress (
C Parameters
     1 epsp, depsp, temp,
C Constants of the constitutive law
     2 parA, parB, parn, pardepsp0, parT0, parTm, parTrec
     3 parp, parm1, parm2, parc1, parc2, epsp00 )
      include 'vaba_param.inc'
C tanh(T)
      tanhT = tanh((temp - parT0) / (parTrec - parT0))      
C part D      
      partD = 1.0 - tanhT * parp * epsp / (1.0 + parp * epsp)      
C stress term 1
      stressTerm1 = (parA + parB * epsp**parn) * (partD + 
     1  (1 - partD) * tanh(1 / ( epsp + epsp00 )))
C stress term 2
      stressTerm2 = 1.0
      if (temp > parT0) then
        if (temp < parTm) then
          stressTerm2 = 1.0 - parm1*((temp-parT0)/(parTm-parT0))**parm2
        else
          stressTerm2 = 0.0
       end if  
      end if   
C stress term3
      stressTerm3 = 1.0
      if (temp > 0.0) then
        CT = parc1 * exp(parc2*temp/parTm) / (temp/parTm)
      else
        CT = 0.0
      end if  
      if (depsp .gt. pardepsp0) then
        stressTerm3 = 1.0 + CT* log (depsp/pardepsp0)
      end if
C Compute and return the yield stress
      yieldStress =  stressTerm1 * stressTerm2 * stressTerm3
      return
      end

C **********************************************************************
C Function to compute the modified TANH hardening
C **********************************************************************
      function yieldHard (
C Parameters
...
C part D      
      partD = 1.0 - tanhT * parp * epsp / (1.0 + parp * epsp) 
C sech2(epsp)
      sech2e = 1.0 / (cosh( 1.0 / (epsp + epsp00)))**2.0            
C stress term 1
      stressTerm1 = (parA + parB * epsp**parn) * (partD + 
     1  (1 - partD) * tanh(1 / ( epsp + epsp00 )))
C stress term 2
...
C hard term1
      hardTerm1 = parn * parB * (epsp**(parn - 1.0)) * (partD + 
     1  (1-partD)*tanh(1/( epsp + epsp00 ))) * stressTerm2 * stressTerm3
C hard term2
      hardTerm2 = (parA+parB*epsp**parn)*(tanh(1.0/(epsp+epsp00))-1.0) 
     1    * parp/(parp*epsp+1.0)**2.0 * tanhT *stressTerm2 *stressTerm3
C hard term3
      hardTerm3 = (parA + parB * epsp**parn) * (partD-1.0)  
     1  * sech2e /((epsp+epsp00)**2.0) * stressTerm2 * stressTerm3
C hard term4
      if (depsp .gt. pardepsp0) then
        hardTerm4 = CT / (dt * depsp) * stressTerm1 * stressTerm2
      else
        hardTerm4 = 0
      end if    
C Compute and return the yield stress
      yieldHard = hardTerm1 +hardTerm2 + hardTerm3 + hardTerm4
      return
      end
...
C using the default initial value of gamma
     ...
     1        parA, parB, parn, pardepsp0, parT0, parTm, parTrec
     2        parp, parm1, parm2, parc1, parc2, epsp00 )
          end if
...
C Increment on the gamma value for Bisection
              gamma = 0.5 * (gammaMax + gammaMin)              
C Algorithm converged, end of computations
              if (abs(gammaMax - gammaMin) < tolNR) irun = 0
C Update the values of epsp and depsp for next loop
              depsp = sqrt23*gamma/dt
              epsp = stateOld(k,1) + sqrt23*gamma
