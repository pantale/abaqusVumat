...
C      STATE(*,6) = total number of Bisection iterations
...
C **********************************************************************  
C Define precision of the Bisection algorithm
...
C Check that Bisection tolerance is OK
        if (epsilon(TolNR) > TolNR) then
          write (*,*) "Precision requested for Bisection"
...
C Initialisations for the Bisection routine
            irun = 1
C Main loop for the Bisection procedure
            do while (irun .eq. 1)
...
C Break with no convergence !!
                write (*,*) "NO CONVERGENCE in Bisection"
...
C Store the new yield stress of the material
          stateNew(k,4) = yield
C Store the number of Bisection iterations
