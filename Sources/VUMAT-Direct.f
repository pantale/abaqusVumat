...
C Compute hardening parameter using right form
            if (epsp > 0.0) then
...
C Compute three hardening parameters
              hardEpsp = yieldHardEpsp(gammaInitial, depsp, temp,
     1          parA, parB, parC, parn, parm, pardepsp0, parT0, parTm)
              hardDepsp = yieldHardDepsp(gammaInitial, depsp, temp,
     1          parA, parB, parC, parn, parm, pardepsp0, parT0, parTm)
              hardTemp = yieldHardTemp(gammaInitial, depsp, temp,
     1          parA, parB, parC, parn, parm, pardepsp0, parT0, parTm)
C Compute the hardening coefficient
              hard = hardEpsp + hardDepsp/dt + heatFr * yield * hardTemp
            end if
C Compute the gamma correction            
            gamma = (Snorm - sqrt23*yield)/(twoG*(1.0+hard/(1.5*twoG)))
C Update plastic strain and plastic strain increment        
            depsp = sqrt23 * gamma / dt
            epsp = stateOld(k,1) + sqrt23 * gamma
            temp = tempInit + 0.5 * gamma * heatFr * 
     1        (sqrt23 * yield + Snorm0)
C Compute new yield stress            
            yield = yieldStress(epsp, depsp, temp,
     1        parA, parB, parC, parn, parm, pardepsp0, parT0, parTm)            
